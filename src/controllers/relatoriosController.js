...
async function listarLicencas() {
  try {
    ...
    for (const colunaValor of colunasValor) {
      if (
        colunaValor === "id" ||
        colunaValor === "distribuidora" ||
        colunaValor === "idDistribuidora" ||
        colunaValor === "slug" ||
        colunaValor === "nomeExibicao" ||
        colunaValor === "idCustoLicenca" ||
        colunaValor === "licencas" ||
        colunaValor === "dataHoraCriacao" ||
        colunaValor === "createdAt" ||
        colunaValor === "updatedAt"
      ) {
        ...
        if (month !== "usuarios") {
          ...
          for (const licenseType in data[year][month]) {
            if (
              ...
              licenseType !== "updatedAt" &&
              licenseType !== "usuarios"
            ) {
...
async function DiferencaLicecasAtuaisEAnteriores() {
  try {
    ...
    console.error(
      "Erro ao obter a diferença entre as licenças atuais e anteriores:",
      error
    );
    throw error;
  }
...
async function calcularHistoricoCustoTotalAnoEMes() {
  try {
        ...
        if (mes !== "usuarios") {
          ...
          for (const licenca in mesQuantidade) {
            if (licenca !== "usuarios" && !licencasCalculadas[licenca]) {
              ...
              custoTotalPorLicenca[ano][mes][licencaNome] +=
    ...
    throw new Error(
      "Ocorreu um erro ao calcular o custo total por licença: " + error.message
    );
  }
}
...
async function calcularCustoTotalMesAnterior() {
  try {
    ...
    if (anoAnteriorData) {
      const mesAnteriorData = anoAnteriorData.find(
        (item) => item.mes === mesAnterior
      );
      if (mesAnteriorData) {
  ...
  try {
    ...
    for (const ano in historicoCustos) {
      ...
      for (const mes of meses) {
        ...
        const totalMes = Object.values(custosMes).reduce(
          (acc, valor) => acc + valor,
          0
        );
...
async function calcularQuantidadeUsuarios() {
  try {
    ...
    const quantidadeUsuarios = usuariosPorDistribuidora.reduce(
      (total, registro) => {
...
async function calcularValorTotalDeCadaLicencaAno() {
  try {
    ...
    throw new Error(
      "Erro ao calcular o histórico do valor total de cada licença por ano: " +
        error.message
    );
  }
}
...
async function DiferencaDoMesAtualComPassado() {
  try {
    ...
    throw new Error(
      "Erro ao calcular a diferença do mês atual com o passado: " +
        error.message
    );
  }
}
...
async function QuantidaDeUsuariosMes() {
  try {
    ...
    throw new Error(
      "Erro ao calcular a quantidade de usuários no mês: " + error.message
    );
  }
}
...
async function DiferencaLicencaAtualComPassado() {
  try {
    ...
    throw new Error(
      "Erro ao calcular a diferença da licença atual com o passado: " +
        error.message
    );
  }
}
...
async function CalcularValorMedioPorPessoaMes() {
  try {
    ...
    throw new Error(
      "Erro ao calcular o valor médio por pessoa por mês: " + error.message
    );
  }
}
